...
//echo "$last_period <br>";
	$arr = ttpro::newfound('Newly Found ' . substr($last_period,0,1));
...
<table width="100%"  ><tr><td rowspan="2" valigh="bottom"><h1>Business and Development</h1></td>
...
function pre_count(dt , pe) {
	...
	var top_data = [
		[ "CreateDate", "TTProList" , "Remark"],
		[ "2016/5/18" , "18967,30704,30744,32093,31887" , ""],
		[ "2016/5/25" , "30704,30744,32093,31887" , ""],
	];
	show_statistics(top_data);

	if (typeof google !== 'undefined' ) {
	google.load("visualization", "1", {packages:["corechart"]});
	google.setOnLoadCallback(drawVisualization);
	}
function file_update(bd_data)
{
...
function busdev_toptable(t1)
{
	...
	for (var i = 1 ; i < t1.length  ; i++) {
		...
		for ( var j = 0 ; j < pp.length; j++) {
			...
			if (hdr[j].includes("TTProList") ) {
				td1.innerHTML = "<a href='defects.php?defects=" + pp[j] + "'>" + pp[j] + "</a></td>";
			} else {
				td1.innerHTML = pp[j];
			}
			td1.id = j;
