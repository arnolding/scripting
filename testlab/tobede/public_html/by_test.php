...
<table border="0px solid black" width="100%"  ><tr><td width="35%" class="h2_bottom" rowspan="2" align=right>
<font size=6>AE Responsible</font></td>
<td colspan="2" valign=bottom align=right id='duration_selector'> <b> Duration: </b>
</td></tr>
...
</td><td rowspan="2" bgcolor='#ffa323' align="center">
<button class="dropbtn" style="height:40" id="change_prio" onClick="priority_redraw()" >Redraw</button>
</td></tr>
<tr>
<td align=right bgcolor='#ffa323'>
<button bgcolor='#d3f3d3' onClick="display_legend()">Legend</button>
<button bgcolor='#d3f3d3' onClick="window.location.href='#st_list';">Table</button>
<div class="dropdown">
  <button class="dropbtn" id=sortx>Sort X-axis</button>
  <div class="dropdown-content">
    <a onclick="sorton('NotReviewed')">NotReviewed</a>
    <a onclick="sorton('Reviewed')">Reviewed</a>
    <a onclick="sorton('Processing')">Processing</a>
    <a onclick="sorton('ToBeVerified')">ToBeVerified</a>
    <a onclick="sorton('Done')">Done</a>
    <a onclick="sorton('Total')">All</a>
    <a onclick="sorton('FirstName')">First Name</a>
    <a onclick="sorton('LastName')">Last Name</a>
  </div>
</div></td>
...
function sorton( sort_c)
{
	...
	sort_x = sort_c;
	document.getElementById("sortx").innerHTML = "Sort X-axis on " + sort_c;
	drawStatus();
}
...
//					alert(radios[i].value);
					...
					if (query_str.indexOf('last') >= 0) {
						...
						location.href = 'by_test.php' + goto_loc;
...
//	data.addColumn('number' , 'Unknown (' + product_stat['subtotal'].Unknown + ')', 'Unknown');
	var i;
	for (i = 0 ; i < chart_seq.length ; i++) {
//	for (var pp in product_stat) {
		var pp = chart_seq[i][0];
	...
	var chart_width = chart_seq.length * 30 + 300;
	if (chart_width < 1200 ) {
		chart_width = 1200;
	}
