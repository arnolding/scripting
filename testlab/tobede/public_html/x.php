...
<b>Duration: </b><input type=radio name=period style='color:green' onClick="location.href='x.php?last=Week'" value="Week" <?php if ($last_period == "Week") echo "checked" ?> >Week
<input type=radio name=period style='color:green' onClick="location.href='x.php?last=BiWeek'" value="BiWeek" <?php if ($last_period == "BiWeek") echo "checked" ?> >BiWeek
<input type=radio name=period style='color:blue' onClick="location.href='x.php?last=Month'" value="Month" <?php if ($last_period == "Month") echo "checked" ?>>Month
<input type=radio name=period style='color:black' onClick="location.href='x.php?last=Year'" value="Year" <?php if ($last_period == "Year") echo "checked" ?>>Year
<!-- <input type=radio name=period style='color:black' onClick="period_chg();location.href='x.php?last=W'" value="N">None -->
</td></tr>
...
<div id='debug'></div>
<table><tr><td id='left_arrow'></td><td valign=bottom align=right bgcolor='#d3f3d3'><b>Priority:</b>&nbsp;
...
</td><td>&nbsp;&nbsp;&nbsp;</td><td id='analysis_pos'></td></tr></table> 
<div id="chart_reference" ></div><br>
...
// major global variables
	...
	var level_analysis_pos = ["Product","",""];
	var all_analysis_pos = ["Reference", "Product","Entered by","Assigned To User"];
...
//	data.addColumn('number' , 'Unknown (' + reference_stat['subtotal'].Unknown + ')', 'Unknown');
	...
	var chart_width = seq.length * 13 + 100;
	if (chart_width < 1200 ) {
		chart_width = 1200;
	}
...
{
	var element_id = "myDropdown" + level;
    document.getElementById(element_id).classList.toggle("show");
}

function update_analysis_pos()
{
	var pos_str = "";
	for (i in level_analysis_pos) {
		pos_str += "<div class='w3-dropdown-hover'>";
		pos_str += "<button onclick='myFunction(";
		pos_str += i + ")' class='w3-btn'>";
		pos_str += level_analysis_pos[i] + "&gt;</button>";
		pos_str += "<div id='myDropdown";
		pos_str += i + "' class='w3-dropdown-content w3-border'>";
		for (j in all_analysis_pos) {
			if (all_analysis_pos[j] == level_analysis_pos[i]) {continue;}
			pos_str += "<a href='#home'>";
			pos_str += all_analysis_pos[j] + "</a>";
		}
		pos_str += "</div></div>";
	}
	document.getElementById("analysis_pos").innerHTML = pos_str;	
}
function drawStatus() {
  // Some raw data (not necessarily accurate)
...
function table_statistics()
{
    ...
    for (var i = top_nr.length - 1 ; i >=0  ; i--) {
		...
		var ppx = pp.replace(/'/ , "&#39");
        td0.innerHTML = pp + "&nbsp;<input type='checkbox' id='ana_" + ppx + "'>";
...
function go_left_arrow()
{
	
	for ( var i in selection_pp) {
		selection_pp[i]=false;
	}
