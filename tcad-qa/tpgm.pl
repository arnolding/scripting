...
sub get_test_result
{
	...
	for my $d1 (@sel_d) {
		...
		if ( not -l $d1 and -f "$d1/r.log" and -f "$d1/result.log") {
			print "$d1/r.log" , "\n" ;
...
sub insert_test_ps
{
	...
	my $sth;
	my $sql = "insert into test_ps (test_id, casename, fname, memory_MB, 
...
again:
	...
	if (!$exe_res) {
		...
		print "tobe_run [$tobe_run] SQL [" . substr($sql,0, 100) . "]\n";
...
#		die;
	...
	#		time, ctime, start, elapse_time, pid, ppid) values ...
    push @table , [ $case, $ps_tree->{fname}, $ps_tree->{size} , $ps_tree->{time} , $ps_tree->{ctime}, $ps_tree->{start}, $ps_tree->{end} - $ps_tree->{start}, $ps_tree->{pid}, $ps_tree->{ppid}];
...
sub make_tree
{
    ...
    for my $i (0 .. $#tA) {
		...
		if ($cmndl =~ /^\S+\/lib\/\w+\/(\d+\.\d+\.\d+\.[ABCR])\/\S+\/(\S+)/) {
			my $ver = $1;
			my $pgm = $2;
			push @tools, "ver [$ver] pgm [$pgm] fname $ps1x->{fname}";
		} else {
