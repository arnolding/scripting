...
require Exporter;
our @ISA = qw(Exporter);
our @EXPORT = qw(logx logname logdb logdb2 logdb3 logdb4 open_client);
my $log_fname = 'nono';
...
sub logdb
{
	my $type = shift;
	my $case_path = shift;
#	/site/silvaco/lib/deckbuild/4.4.3.R/x86_64-linux/deckbuild.exe
	my $pname = shift;
	my $pid = shift;
	return 0 unless ($logdb_arguments{$type});
	...
	my $platform = g_platform();
	my $hostname = g_hostname();
	my $tester = `whoami`; chomp $tester;
	my ($arch , $num_cpu, $CPU_MHz) = g_cpu();
	my ($install, $prod, $ver, $prod_platform, $prod_exe) =
		('NA','NA','0.0.0.0','NA','NA');
	if ($pname =~ /(\S+)\/lib\/(\S+)\/(\S+)\/(\S+)\/(\S+)$/) {
		($install, $prod, $ver, $prod_platform, $prod_exe) =
		($1, $2, $3, $4, $5);
	}
	my ($memTotal , $swapTotal ) = g_hostmem();
	...
	$sql .= $logdb_arguments{$type}->[1];
	$sql .= "VALUES (";
	$sql .= " datetime('" . $now_string . "','unixepoch'),";
	$sql .= "'" . $tester . "',";
	$sql .= $pid .",";
	$sql .= "'" . $case_path . "',";
	$sql .= "'N',";
...
print "logdb: [$sql]\n";
	...
	$sql = "SELECT run_id from $logdb_arguments{$type}->[0] WHERE hostname='$hostname' AND pid=$pid order by run_id desc limit 1;";
	print $socket "$db_fname \"$sql\"\n";
	my $run_id = 0;
	while (defined ($line = <$socket>)) {
		chomp $line;
