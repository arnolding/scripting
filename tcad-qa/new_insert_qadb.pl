...
warn "pgm [$pgm] test_date [$test_date]" if (not defined $pgm);
		...
		my $tool_id_table = tool_id($test_id, $idx1->{ptree}->{summary_table});
...
sub insert_test_idx
{
	...
	if ($data) {
		...
		print "no test_id but mach_id $mach_id\n";
...
sub tool_id
{
	...
	my $sth;
	my $sql = "insert into test_ps (test_id, casename, memory_MB, 
			time, ctime, start, elapse_time, pid, ppid, num_processor, tool_id) values ";
...
sub make_tree
{
    ...
    for my $i (0 .. $#tA) {
		...
		$ps1x->{short_name} = undef;
		$ps1x->{tool_platform} = undef;
		$ps1x->{program_name} = undef;
		...
		if ($cmndl =~ /^\S+\/lib\/(\w+)\/(\d+\.\d+\.\d+\.[ABCR])\/(\S+)\/(\S+) (-P \d+)?/) {
			...
			my $pgm = $4;
			my $num_p = $5 || "";
			my $tool_path = "lib/" . $short_name . "/" . $ver . "/" . $tool_platform . "/" . $pgm;
			push @tools, "ver [$ver] pgm [$pgm] fname $ps1x->{fname} $num_p";
			$ps1x->{short_name} = $short_name;
			$ps1x->{tool_platform} = $tool_platform;
			$ps1x->{program_name} = $pgm;
			$ps1x->{version} = $ver;
			if ($num_p) {
