...
{
	my $run_tag = shift;
	my $running_path = "$run_tag/output/running";
	if (! -d $running_path) {
		warn "$running_path does not exist";
		return undef;
	}
	my @running_folder_path = ($running_path);
#/build/arnoldh/reg/tests/tcad-0929-0/cases/4.2.9.R/*/*/.*/deckbuild/rto.xml";
	find(\&find_file_done, @running_folder_path);

	print "number of stat-done.log " . scalar @$stat_done_aref . " in $run_tag\n";
}
sub read_ptree
{
	my $run_tag = shift;
...
#	close P;
	...
	my $reg_check = check_ps_summary($ps_set);
	my $reg_summary; ## = get_ps_summary($ps_set);
	
#a	open P , "> $output_path/reg_summary.log";
#a	print P Data::Dumper->Dump([$reg_summary]);
#a	close P;
	my $sum_table; ## = summary_in_table($reg_summary);
#a	open P, "> $output_path/reg_ptable.summary";
#a	print P Data::Dumper->Dump([$sum_table]);
#a	close P;
...
sub get_test_result
{
	...
	for my $d1 (@sel_d) {
		if ( not -l $d1 and -f "$d1/r.log" and -f "$d1/result.log" and -e "$d1/output") {
			...
			read_running($d1);
next;
#a			my $reg_ptree = read_ptree($d1);
#			read_ptree($d1);
			my $reg_ptree = "1";
			next unless $reg_ptree;
			print "$d1/r.log" , "\n" ;
#aa			my $r_href = parse_r_file("$d1");
			my $r_href = "2";
			my $res_href = parse_result_file("$d1");
			$test_dir = {
				test_idx => [{
					path => $d1,
					r_log => $r_href,
...
sub performance2hash
{
	my $perf_str = shift;
	if ($perf_str =~ /([\d\.]+) usertime ([\d\.]+) systime ([\d\.:]+) elapsed ([\d\.]+)%CPU/) {
		...
		die "wrong performance str : [$perf_str]";
	}
}
sub time2sec
{
	...
	if ($time_str !~ /\d\d:\d\d:\d\d/) {
		die "wrong time str : [$time_str], must be hh:mm:ss";
	}
	my @ss = split(":" , $time_str);
	return $ss[0]*3600 + $ss[1]*60 + $ss[2];
}
sub parse_result_file
{
	my $r_dir = shift;
	my $res_fname = $r_dir . "/result.log";
	open FN , "< $res_fname" or die "open result file $res_fname failed";
	...
	while (<FN>) {
		chomp;
		my $line = $_;
		my ($case_tag, $start_time, $end_time, $performance_str, $windows_str, $all_windows) = split(/, / , $line);
		my $start_time_sec = time2sec($start_time);
		my $end_time_sec = time2sec($end_time);
		my $perf_time = performance2hash($performance_str);
		my $calc_elapse = $end_time_sec - $start_time_sec;
		if ($calc_elapse < 0) {
			warn "Seems leap to another day start time [$start_time] and end [$end_time]";
		}
		$r_data->{$case_tag} = {
			start			=> $start_time,
			end				=> $end_time,
			calc_elapsed	=> $calc_elapse,
			time_str		=> $perf_time,
		};
...
warn "pgm [$pgm] test_date [$test_date]" if (not defined $pgm);
		...
		my $testps_id = insert_test_ps($test_id, $idx1->{ptree}->{summary_table});
		closedb();
	}
}
...
sub check_ps_summary
{
    my $ps_set = shift;
    my @tmp_ps;
    for my $ps1 (@$ps_set) {
        my $href = check_ps1_summary($ps1);
#        push @tmp_ps , $href;
    }
...
sub ps_issue_finding
{
    my $ptree_pre = shift;
    my $ps_epoch_pre = shift;
    my $ptree = shift;
    my $ps_epoch = shift;
    my $cover = 1;
    ...
    for my $c_pre1 (@pre) {
		...
		for my $c_new1 (@cur) {
			if (( $c_new1->{pid} == $c_pre1->{pid}) and
                ($c_new1->{start} == $c_pre1->{start}) ) {
				$time_more += $c_new1->{time} - $c_pre1->{time} + $c_new1->{ctime} - $c_pre1->{ctime};
				$found = 1;
				push @{$href->{accu}} , {
					pid	=> $c_new1->{pid},
					ctime_d	=> $c_new1->{ctime} - $c_pre1->{ctime},
					time_d	=> $c_new1->{time} - $c_pre1->{time}
				};
				last;
			}
		}
	...
	my $utilization = $time_more * 0.000001 / ($ps_epoch - $ps_epoch_pre);
	$href->{util} = $utilization;
    return $href;
}
sub get_ps1_summary
{
...
sub check_ps1_summary
{
    ...
    for my $ps_shot1 (@ps_shot) {
        #warn "$ps_case";
        my $shot_epoch = $ps_shot1->{shot_epoch};
		push @shot_epoch_a , $shot_epoch;
		my $ps_lead = $ps_shot1->{ps_tree};
		
        if ( defined $ps_lead_pre) {
            my $pp = ps_issue_finding($ps_lead_pre, $shot_epoch_pre, $ps_lead, $shot_epoch);
			if ($pp->{util} < 0.5) {
				print "$ps_case\n";
				print Data::Dumper->Dump([$pp]);
			}
        }
        $ps_lead_pre = $ps_lead;
		$shot_epoch_pre = $shot_epoch;
    }
...
{
    my $fname = $File::Find::name;
    if ($fname =~ /stat-done\.log$/ ) {
        #print $fname , "\n";
        push @$stat_done_aref , $fname;
    }
}
